...
- `crates/helper`: Dependency-leaf cancellation, fair helper admission, RAII
  job permits, bounded pipe capture, inherited descriptors, private process
  groups, deadlines, termination escalation, and child reaping. Media and
  request policy must not depend on this crate.
- `crates/protocol`: Stable wire constants and pure protocol behavior: renderer
  profiles, media formats, persisted stream-metadata grammar, object IDs,
  paths, dates, HTTP persistence, SOAP names, and SSDP service types. Changes
  here can affect every client and stored catalog.
...
- `contrib/library/`: Operator tools that write NFO, posters, generated
  `genres/` views, and `.rusty_previews/` beside media. rustyDLNA does not run
  them. Library root is `--root` / `RUSTY_DLNA_MEDIA`; caches live in
  `<library>/.rusty-library/`. Never store provider credentials in this tree.
...
- `docs/OPERATIONS.md`: Storage, health, alerts, recovery, native service use,
  and the operator library-maintenance tools.
...
The canonical gate includes the dependency-free web unit suite and requires
Node.js 20 or newer plus npm. CI and release validation additionally install
the locked npm dependencies and run the full Playwright project matrix.
