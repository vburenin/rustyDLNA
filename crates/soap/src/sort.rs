...
/// Maximum decoded `SortCriteria` argument size accepted from a SOAP request.
pub const MAX_SORT_CRITERIA_BYTES: usize = 1024;
/// Maximum number of sort keys accepted from a SOAP request.
...
pub fn parse_sort_criteria(raw: Option<&str>) -> Result<Vec<SortSpec>, SortError> {
    if raw.is_some_and(|value| value.len() > MAX_SORT_CRITERIA_BYTES) {
        return Err(SortError::Unparseable);
    }
...
mod tests {
    ...
    #[test]
    fn sort_criteria_byte_limit_is_inclusive() {
        let field = "+dc:title";
        let exact = format!(
            "{}{}",
            " ".repeat(MAX_SORT_CRITERIA_BYTES - field.len()),
            field
        );
        assert_eq!(parse_sort_criteria(Some(&exact)).unwrap().len(), 1);

        let over = format!(" {exact}");
        assert_eq!(
            parse_sort_criteria(Some(&over)),
            Err(SortError::Unparseable)
        );
    }

    #[test]
    fn sort_criteria_key_limit_is_inclusive() {
        let exact = std::iter::repeat_n("+dc:title", MAX_SORT_KEYS)
            .collect::<Vec<_>>()
            .join(",");
        assert_eq!(
            parse_sort_criteria(Some(&exact)).unwrap().len(),
            MAX_SORT_KEYS
        );

        let over = format!("{exact},+dc:title");
        assert_eq!(
            parse_sort_criteria(Some(&over)),
            Err(SortError::Unparseable)
        );
