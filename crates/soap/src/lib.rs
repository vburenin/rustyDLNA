...
use rusty_dlna_protocol::escape_xml_text_into as xml_escape_didl_into;
...
use rusty_dlna_protocol::soap::{
    ...
    SEARCH_CAPS, SOAP_METHODS, SORT_CAPS,
};
...
pub use sort::{
    default_order, parse_sort_criteria, sort_or_709, DefaultOrder, SortKey, SortSpec,
    MAX_SORT_CRITERIA_BYTES, MAX_SORT_KEYS,
};

pub use rusty_dlna_protocol::escape_xml as xml_escape;
...
pub use rusty_dlna_protocol::escape_xml_text as xml_escape_didl_result;
...
const MAX_SOAP_XML_DEPTH: usize = 128;

fn xml_nodes(hay: &str) -> Result<Vec<XmlNode>, SoapXmlError> {
            ...
            Ok(Event::End(end)) => {
                ...
                if node.name != end_name {
                    return Err(SoapXmlError(format!(
                        "closing element {end_name} does not match {}",
                        node.name
                    )));
                }
    ...
    while let Some((node, visited)) = pending.pop() {
        if visited {
            fields
                ...
                .or_default()
                .push(node.value.trim().to_string());
            continue;
        }
...
pub struct SoapCall {
    ...
    pub requested_count: Option<i32>,
...
fn take_unique_soap_field(
    fields: &mut HashMap<String, Vec<String>>,
    name: &str,
) -> Result<Option<String>, SoapXmlError> {
    let Some(values) = fields.remove(&name.to_ascii_lowercase()) else {
        return Ok(None);
    };
    let mut values = values.into_iter();
    let first = values.next();
    if let Some(first_value) = first.as_ref() {
        if values.any(|value| value != *first_value) {
            return Err(SoapXmlError(format!(
                "{name} has conflicting duplicate values"
            )));
        }
    }
    Ok(first)
...
const SOAP_PARAMETER_NAMES: &[&str] = &[
    "objectid",
    "containerid",
    "browseflag",
    "startingindex",
...
fn action_name_from_header(action: &str) -> Option<&str> {
    let rest = action.split_once('#')?.1;
    let name = rest.trim_matches(|character| character == '"' || character == '\'');
    (!name.is_empty()).then_some(name)
}

fn node_is_known_action(node: &XmlNode) -> bool {
    SOAP_METHODS
        .iter()
        .any(|method| node.name.eq_ignore_ascii_case(method))
}

fn soap_parameter_nodes<'a>(
    roots: &'a [XmlNode],
    action: &str,
    method: Option<&'static str>,
) -> Result<&'a [XmlNode], SoapXmlError> {
    let expected = method.or_else(|| action_name_from_header(action));
    let raw_action_name = action_name_from_header(action);
    let matches_action = |node: &XmlNode| {
        expected.is_some_and(|expected| node.name.eq_ignore_ascii_case(expected))
            || raw_action_name
                .is_some_and(|raw_action_name| node.name.eq_ignore_ascii_case(raw_action_name))
    };
    if roots.is_empty() {
        return Ok(roots);
    }

    if roots.len() == 1 && roots[0].name.eq_ignore_ascii_case("Envelope") {
        let envelope = &roots[0];
        let mut bodies = envelope
            .children
            .iter()
            .filter(|node| node.name.eq_ignore_ascii_case("Body"));
        let body = bodies
            .next()
            .ok_or_else(|| SoapXmlError("SOAP Envelope has no Body".into()))?;
        if bodies.next().is_some() {
            return Err(SoapXmlError(
                "SOAP Envelope has multiple Body elements".into(),
            ));
        }
        if body.children.len() != 1 {
            return Err(SoapXmlError(
                "SOAP Body must contain exactly one action".into(),
            ));
        }
        let expected = expected
            .ok_or_else(|| SoapXmlError("SOAPAction does not name the body action".into()))?;
        let body_action = &body.children[0];
        if !matches_action(body_action) {
            return Err(SoapXmlError(format!(
                "SOAP body action {} does not match SOAPAction {expected}",
                body_action.name
            )));
        }
        return Ok(&body_action.children);
    }

    if roots.len() == 1 {
        let root = &roots[0];
        if matches_action(root) {
            return Ok(&root.children);
        }
        if SOAP_PARAMETER_NAMES.contains(&root.name.as_str()) {
            return Ok(roots);
        }
        if node_is_known_action(root) || !root.children.is_empty() {
            return Err(SoapXmlError(format!(
                "SOAP body action {} does not match SOAPAction",
                root.name
            )));
        }
        return Ok(roots);
    ...
    if roots.iter().any(|root| {
        root.name.eq_ignore_ascii_case("Envelope")
            || node_is_known_action(root)
            || matches_action(root)
            || !root.children.is_empty()
    }) {
        return Err(SoapXmlError(
            "SOAP body contains multiple top-level actions".into(),
        ));
    }
    Ok(roots)
}

fn direct_xml_fields(nodes: &[XmlNode]) -> HashMap<String, Vec<String>> {
    let mut fields: HashMap<String, Vec<String>> = HashMap::new();
    for node in nodes {
        fields
            .entry(node.name.clone())
...
pub fn try_parse_soap_call(action: &str, body: &str) -> Result<SoapCall, SoapXmlError> {
    let method = method_from_header(action);
    let roots = xml_nodes(body)?;
    let parameters = soap_parameter_nodes(&roots, action, method)?;
    let mut fields = direct_xml_fields(parameters);
    let object_id = take_unique_soap_field(&mut fields, "ObjectID")?;
    let container_id = take_unique_soap_field(&mut fields, "ContainerID")?;
    let object_id = match (object_id, container_id) {
        (Some(object_id), Some(container_id)) if object_id != container_id => {
            return Err(SoapXmlError(
                "ObjectID and ContainerID have conflicting values".into(),
            ));
        }
        (Some(object_id), _) => Some(object_id),
        (None, container_id) => container_id,
    };
    let parse_i32 = |name: &str, value: Option<String>| -> Result<Option<i32>, SoapXmlError> {
        match value {
            None => Ok(None),
            Some(value) => value
                .trim()
    ...
    let starting_index = parse_i32(
        "StartingIndex",
        take_unique_soap_field(&mut fields, "StartingIndex")?,
    )?;
    let requested_count = parse_i32(
        "RequestedCount",
        take_unique_soap_field(&mut fields, "RequestedCount")?,
    )?;
    let pos_second = match take_unique_soap_field(&mut fields, "PosSecond")? {
        None => None,
        Some(value) => {
            Some(i64::from(value.trim().parse::<i32>().map_err(|_| {
                SoapXmlError("PosSecond is not a valid integer".into())
            })?))
        }
    };
    Ok(SoapCall {
        method,
        object_id,
        browse_flag: take_unique_soap_field(&mut fields, "BrowseFlag")?,
        starting_index,
        requested_count,
        search_criteria: take_unique_soap_field(&mut fields, "SearchCriteria")?,
        pos_second,
        connection_id: take_unique_soap_field(&mut fields, "ConnectionID")?,
        device_id: take_unique_soap_field(&mut fields, "DeviceID")?,
        filter: take_unique_soap_field(&mut fields, "Filter")?,
        current_tag_value: take_unique_soap_field(&mut fields, "CurrentTagValue")?,
        new_tag_value: take_unique_soap_field(&mut fields, "NewTagValue")?,
        sort_criteria: take_unique_soap_field(&mut fields, "SortCriteria")?,
        var_name: take_unique_soap_field(&mut fields, "varName")?,
    })
}
...
pub use rusty_dlna_protocol::PROTOCOL_INFO_SOURCE;

/// Delegates to the protocol crate while preserving the SOAP crate API.
pub fn protocol_info_source() -> String {
    rusty_dlna_protocol::protocol_info_source()
}
...
mod tests {
    ...
    #[test]
    fn protocol_info_compatibility_exports_match_the_protocol_owner() {
        assert_eq!(
            PROTOCOL_INFO_SOURCE,
            rusty_dlna_protocol::PROTOCOL_INFO_SOURCE
        );
        assert_eq!(
            protocol_info_source(),
    ...
    #[test]
    fn soap_call_rejects_conflicting_duplicate_fields() {
        for body in [
            "<Browse><ObjectID>0</ObjectID><ObjectID>1</ObjectID></Browse>",
            "<Browse><ContainerID>0</ContainerID><ContainerID>1</ContainerID></Browse>",
            "<Browse><ObjectID>0</ObjectID><ContainerID>1</ContainerID></Browse>",
            "<Browse><BrowseFlag>BrowseMetadata</BrowseFlag><browseflag>BrowseDirectChildren</browseflag></Browse>",
            "<Browse><StartingIndex>0</StartingIndex><StartingIndex>1</StartingIndex></Browse>",
            "<QueryStateVariable><varName>A_ARG_TYPE_UpdateID</varName><VARNAME>SystemUpdateID</VARNAME></QueryStateVariable>",
        ] {
            assert!(
                try_parse_soap_call("urn:x#Browse", body).is_err(),
                "conflicting SOAP arguments were accepted: {body}"
            );
        }
    }

    #[test]
    fn soap_call_accepts_identical_duplicates_and_compatible_fragments() {
        let repeated = try_parse_soap_call(
            "urn:x#Browse",
            "<Browse><ObjectID>0</ObjectID><objectid>0</objectid><ContainerID>0</ContainerID><StartingIndex>01</StartingIndex><startingindex>01</startingindex></Browse>",
        )
        .expect("identical duplicate values");
        assert_eq!(repeated.object_id.as_deref(), Some("0"));
        assert_eq!(repeated.starting_index, Some(1));

        let bare = try_parse_soap_call(
            "urn:x#Browse",
            "<ObJeCtId>64$1</ObJeCtId><BrOwSeFlAg>BrowseMetadata</BrOwSeFlAg>",
        )
        .expect("bare case-insensitive parameter fragments");
        assert_eq!(bare.object_id.as_deref(), Some("64$1"));
        assert_eq!(bare.browse_flag.as_deref(), Some("BrowseMetadata"));
    }

    #[test]
    fn soap_call_scopes_envelope_fields_to_the_matching_body_action() {
        let body = r#"
            <s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/">
              <s:Header><ObjectID>header-value</ObjectID></s:Header>
              <s:Body>
                <u:Browse xmlns:u="urn:schemas-upnp-org:service:ContentDirectory:1">
                  <ObjectID>64$1</ObjectID>
                  <Vendor><ObjectID>nested-a</ObjectID><ObjectID>nested-b</ObjectID></Vendor>
                  <BrowseFlag>BrowseMetadata</BrowseFlag>
                </u:Browse>
              </s:Body>
            </s:Envelope>
        "#;
        let call = try_parse_soap_call("urn:x#Browse", body).expect("scoped SOAP envelope");
        assert_eq!(call.object_id.as_deref(), Some("64$1"));
        assert_eq!(call.browse_flag.as_deref(), Some("BrowseMetadata"));
    }

    #[test]
    fn soap_xml_nesting_is_bounded() {
        let mut body = "<Browse>".to_string();
        for _ in 0..MAX_SOAP_XML_DEPTH {
            body.push_str("<Vendor>");
        }
        for _ in 0..MAX_SOAP_XML_DEPTH {
            body.push_str("</Vendor>");
        }
        body.push_str("</Browse>");
        assert!(try_parse_soap_call("urn:x#Browse", &body).is_err());
    }

    #[test]
    fn soap_call_rejects_mismatched_or_ambiguous_action_elements() {
        for body in [
            "<Search><ObjectID>0</ObjectID></Search>",
            "<VendorAction><ObjectID>0</ObjectID></VendorAction>",
            "<Envelope><Body><Search><ObjectID>0</ObjectID></Search></Body></Envelope>",
            "<Envelope><Body><Browse><ObjectID>0</ObjectID></Browse><Browse><ObjectID>1</ObjectID></Browse></Body></Envelope>",
            "<Envelope><Body><Browse><ObjectID>0</ObjectID></Browse></Body><Body><Browse><ObjectID>1</ObjectID></Browse></Body></Envelope>",
            "<Browse><ObjectID>0</ObjectID></Browse><Browse><ObjectID>1</ObjectID></Browse>",
        ] {
            assert!(
                try_parse_soap_call("urn:x#Browse", body).is_err(),
                "ambiguous or mismatched SOAP action was accepted: {body}"
            );
        }
    }

    #[test]
    fn soap_call_root_action_ignores_nested_fields_but_bare_fragments_still_work() {
        let root = try_parse_soap_call(
            "urn:x#Browse",
            "<BrOwSe><Vendor><ObjectID>nested</ObjectID></Vendor><OBJECTID>direct</OBJECTID></BrOwSe>",
        )
        .expect("case-insensitive root action");
        assert_eq!(root.object_id.as_deref(), Some("direct"));

        let bare = try_parse_soap_call(
            "urn:x#Browse",
            "<Vendor>ignored</Vendor><ObJeCtId>bare</ObJeCtId>",
        )
        .expect("bare top-level parameter fragments");
        assert_eq!(bare.object_id.as_deref(), Some("bare"));

        let prefixed_legacy_action = try_parse_soap_call(
            "urn:x#BrowseDirectChildren",
            "<BrowseDirectChildren><ObjectID>0</ObjectID></BrowseDirectChildren>",
        )
        .expect("legacy prefix action matching");
        assert_eq!(prefixed_legacy_action.method, Some("Browse"));
        assert_eq!(prefixed_legacy_action.object_id.as_deref(), Some("0"));
    }

    #[test]
    fn soap_call_bounds_pos_second_to_the_upnp_i4_type() {
        for value in [i32::MIN, i32::MAX] {
            let body = format!("<X_SetBookmark><PosSecond>{value}</PosSecond></X_SetBookmark>");
            let call = try_parse_soap_call("urn:x#X_SetBookmark", &body).unwrap();
            assert_eq!(call.pos_second, Some(i64::from(value)));
        }
        for value in ["-2147483649", "2147483648"] {
            let body = format!("<X_SetBookmark><PosSecond>{value}</PosSecond></X_SetBookmark>");
            assert!(try_parse_soap_call("urn:x#X_SetBookmark", &body).is_err());
        }
    }
    ...
    #[test]
    fn generated_soap_replaces_invalid_xml_scalars_and_remains_parseable() {
        let invalid_controls = (0_u32..=0x1f)
            .filter(|value| !matches!(value, 9 | 10 | 13))
            .map(|value| char::from_u32(value).expect("C0 scalar"))
            .collect::<String>();
        let expected_title = format!(
            "before{}after",
            "\u{fffd}".repeat(invalid_controls.chars().count())
        );
        let mut object = dummy_item(None, vec![]);
        object.id = "item\u{fffe}\u{ffff}".into();
        object.title = format!("before{invalid_controls}after");

        let soap = build_browse(false, &[object], 1, 1, 7, &FilterBits::standard());
        let document = roxmltree::Document::parse(&soap).expect("sanitized SOAP parses");
        let result = document
            .descendants()
            .find(|node| node.tag_name().name() == "Result")
            .and_then(|node| node.text())
            .expect("Browse Result text");
        let didl = roxmltree::Document::parse(result).expect("sanitized DIDL parses");
        let item = didl
            .descendants()
            .find(|node| node.tag_name().name() == "item")
            .expect("DIDL item");
        assert_eq!(item.attribute("id"), Some("item\u{fffd}\u{fffd}"));
        assert_eq!(
            item.descendants()
        ...
        let fault = soap_fault(402, "bad\u{1}description\u{ffff}");
        let document = roxmltree::Document::parse(&fault).expect("sanitized SOAP fault parses");
        assert_eq!(
            document
                .descendants()
                .find(|node| node.tag_name().name() == "errorDescription")
                .and_then(|node| node.text()),
            Some("bad\u{fffd}description\u{fffd}")
        );
    }
