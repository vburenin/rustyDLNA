...
impl HttpRequest {
    ...
    pub fn parse_headers(raw: &str) -> Result<Self, ParseError> {
        ...
        let mut soap_action_seen = false;
        for line in lines {
            if line.is_empty() {
                continue;
            }
            ...
            let value = trim_http_ows(raw_value);
            ...
            if name.eq_ignore_ascii_case("SOAPAction") {
                if soap_action_seen {
                    return Err(ParseError::DuplicateSoapAction);
                }
                soap_action_seen = true;
            }
...
mod tests {
    ...
    #[test]
    fn rejects_duplicate_soap_action_even_when_values_match() {
        for second in [r#""urn:x#Browse""#, r#""urn:x#Search""#] {
            let raw = format!(
                "POST /ctl HTTP/1.1\r\nHost: 127.0.0.1\r\nSOAPAction: \"urn:x#Browse\"\r\nSOAPAction: {second}\r\n\r\n"
            );
            assert_eq!(
                parse(&raw).unwrap_err(),
                ParseError::DuplicateSoapAction,
                "accepted duplicate SOAPAction {second}"
            );
        }
    }
