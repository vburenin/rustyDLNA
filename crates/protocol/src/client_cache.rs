...
impl ClientCache {
    ...
    pub fn add(
        ...
        for slot in &mut self.slots {
            if slot.is_some_and(|entry| now.saturating_sub(entry.age) > CLIENT_CACHE_TTL_SECS) {
                *slot = None;
            }
        }
...
mod tests {
    ...
    #[test]
    fn full_cache_reclaims_expired_renderers_for_new_addresses() {
        let kodi = identify_user_agent("Kodi/21.0").expect("kodi");
        let mut cache = ClientCache::new();
        for host in 1..=CLIENT_CACHE_SLOTS {
            let ip = Ipv4Addr::new(192, 0, 2, host as u8);
            assert!(cache.add(ip, kodi, None, 0));
        }
        assert_eq!(cache.len(), CLIENT_CACHE_SLOTS);
        assert!(
            !cache.add(Ipv4Addr::new(192, 0, 2, 250), kodi, None, 1),
            "a full cache must retain live renderer entries"
        );
        for host in 13..=CLIENT_CACHE_SLOTS {
            cache.set_age(Ipv4Addr::new(192, 0, 2, host as u8), 1);
        }

        let replacement = Ipv4Addr::new(192, 0, 2, 250);
        let now = CLIENT_CACHE_TTL_SECS + 1;
        assert!(cache.add(replacement, kodi, None, now));
        assert_eq!(cache.len(), 14, "only expired slots are reclaimed");
        assert_eq!(
            cache
                .search(replacement, now, None)
                .map(|profile| profile.kind),
            Some(ClientKind::Kodi)
        );
        assert_eq!(
            cache
                .search(Ipv4Addr::new(192, 0, 2, 13), now, None)
                .map(|profile| profile.kind),
            Some(ClientKind::Kodi),
            "an entry exactly at the TTL boundary remains live"
        );
    }
}
