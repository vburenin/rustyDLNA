...
const MAX_AI_UPSCALE_SHADER_BYTES: u64 = 1024 * 1024;

#[cfg(target_os = "linux")]
fn freeze_ai_upscale_shader(bytes: &[u8]) -> std::io::Result<std::fs::File> {
    use std::io::{Seek, Write};
    use std::os::fd::{AsRawFd, FromRawFd};

    // SAFETY: memfd_create receives a valid static C string and flags. A
    // successful call returns a new descriptor owned by this function.
    let descriptor = unsafe {
        libc::memfd_create(
            c"rustydlna-ai-upscale".as_ptr(),
    ...
    // SAFETY: descriptor was freshly returned by memfd_create and ownership is
    // transferred exactly once to File.
    let mut file = unsafe { std::fs::File::from_raw_fd(descriptor) };
    file.write_all(bytes)?;
    let seals = libc::F_SEAL_SEAL | libc::F_SEAL_SHRINK | libc::F_SEAL_GROW | libc::F_SEAL_WRITE;
    // SAFETY: fcntl operates on the live memfd and the integer seal mask has
    // the F_ADD_SEALS contract. File retains descriptor ownership.
...
fn freeze_ai_upscale_shader(bytes: &[u8]) -> std::io::Result<std::fs::File> {
    ...
    for _ in 0..8 {
        let path =
            std::env::temp_dir().join(format!(".rustydlna-ai-upscale-{}", uuid::Uuid::new_v4()));
    ...
    Err(std::io::Error::new(
        ...
        "could not allocate an anonymous AI-upscale shader file",
    ))
}

fn load_ai_upscale_profiles(
    cfg: &Config,
    config_dir: &Path,
) -> Result<Vec<BrowserAiUpscaleProfile>, AppInitError> {
    ...
    cfg.web
        ...
        .iter()
        .map(|profile| {
            let path = if profile.shader_path.is_absolute() {
                profile.shader_path.clone()
            } else {
                config_dir.join(&profile.shader_path)
            };
            let mut file = std::fs::File::open(&path).map_err(|source| {
                AppInitError::AiUpscaleShader {
                    path: path.clone(),
                    message: source.to_string(),
                }
            })?;
            let metadata = file.metadata().map_err(|source| AppInitError::AiUpscaleShader {
                path: path.clone(),
                message: source.to_string(),
            })?;
            if !metadata.is_file() || metadata.len() == 0 || metadata.len() > MAX_AI_UPSCALE_SHADER_BYTES {
                return Err(AppInitError::AiUpscaleShader {
                    path,
                    message: format!(
                        "expected a non-empty regular file no larger than {MAX_AI_UPSCALE_SHADER_BYTES} bytes"
                    ),
                });
            }
            let capacity = usize::try_from(metadata.len().min(MAX_AI_UPSCALE_SHADER_BYTES))
                .unwrap_or(0);
            let mut bytes = Vec::with_capacity(capacity);
            file.by_ref()
                .take(MAX_AI_UPSCALE_SHADER_BYTES + 1)
                .read_to_end(&mut bytes)
                .map_err(|source| AppInitError::AiUpscaleShader {
                    path: path.clone(),
                    message: source.to_string(),
                })?;
            if bytes.len() as u64 > MAX_AI_UPSCALE_SHADER_BYTES {
                return Err(AppInitError::AiUpscaleShader {
                    path,
                    message: format!(
                        "shader grew beyond the {MAX_AI_UPSCALE_SHADER_BYTES}-byte limit while it was read"
                    ),
                });
            }
            let shader = std::str::from_utf8(&bytes).map_err(|_| AppInitError::AiUpscaleShader {
                path: path.clone(),
                message: "shader is not UTF-8 text".into(),
            })?;
            if !shader.lines().any(|line| line.trim_start().starts_with("//!HOOK")) {
                return Err(AppInitError::AiUpscaleShader {
                    path,
                    message: "shader has no mpv/libplacebo //!HOOK directive".into(),
                });
            }
            let shader_sha256 = Sha256::digest(&bytes)
                .iter()
                .map(|byte| format!("{byte:02x}"))
                .collect();
            let file = freeze_ai_upscale_shader(&bytes).map_err(|source| {
                AppInitError::AiUpscaleShader {
                    path: path.clone(),
                    message: format!("cannot freeze validated shader bytes: {source}"),
                }
            })?;
            Ok(BrowserAiUpscaleProfile {
                name: profile.name.clone(),
                shader_file: Arc::new(file),
                shader_sha256,
...
impl App {
    ...
    pub fn try_from_config(
        ...
        Self::preflight_config(cfg, http_port, ssdp_port, config_dir)?.into_app()
    }

    /// Resolve and validate configuration without opening or changing runtime
    /// storage. The returned value can be inspected by diagnostic commands or
    /// converted into a running application with [`AppPreflight::into_app`].
    pub fn preflight_config(
        cfg: Config,
        http_port: u16,
        ssdp_port: u16,
        config_dir: &Path,
    ) -> Result<AppPreflight, AppInitError> {
        let interfaces = active_ipv4_interfaces();
        let default_route = default_route_interface();
        Self::preflight_config_with_network(
            cfg,
            http_port,
            ssdp_port,
            config_dir,
    ...
    #[cfg(test)]
    pub(crate) fn try_from_config_with_network(
        cfg: Config,
        http_port: u16,
        ssdp_port: u16,
        config_dir: &Path,
    ...
    fn preflight_config_with_network(
        cfg: Config,
        http_port: u16,
        ssdp_port: u16,
        config_dir: &Path,
        ...
        let ai_upscale_profiles = load_ai_upscale_profiles(&cfg, config_dir)?;
        ...
        let media_roots = build_media_roots(&cfg.media_dir, config_dir)
            .map_err(|message| AppInitError::MediaRoots { message })?;
        let configured_uuid = cfg
            .uuid
            .as_deref()
            .map(normalize_uuid)
            .transpose()
            .map_err(|message| AppInitError::Identity {
...
impl AppPreflight {
    /// Exercise each configured shader through the exact Vulkan and NVENC
    /// stages used by browser playback.
    pub fn validate_ai_upscale_tools(&self) -> Result<Vec<String>, ConfigValidationError> {
        ...
        for profile in &self.ai_upscale_profiles {
            let filter = format!(
                concat!(
                    "format=yuv420p,hwupload,",
                    "libplacebo=custom_shader_path={}:format=yuv420p:",
                    "w=640:h=360:force_original_aspect_ratio=decrease:",
                    "force_divisible_by=2,hwdownload,format=yuv420p"
                ),
                rusty_dlna_transcode::BROWSER_AI_UPSCALE_SHADER_CHILD_PATH,
            );
            ...
            let runner = SupervisedCommand::new(&mut command)
                ...
                .inherit_file_at(
                    &profile.shader_file,
                    rusty_dlna_transcode::BROWSER_AI_UPSCALE_SHADER_FD,
                )
                .map_err(|error| {
                    ConfigValidationError::from(format!(
                        "cannot inherit AI-upscale shader {:?}: {error}",
                        profile.name
                    ))
                })?;
            let outcome = runner
                ...
                .map_err(|error| {
                    ConfigValidationError::from(format!(
                        "cannot exercise AI-upscale profile {:?}: {error}",
                        profile.name
                    ))
                })?;
            let output = match outcome {
                SupervisedOutcome::Exited(output) => output,
                SupervisedOutcome::Deadline { stderr, .. } => {
                    return Err(ConfigValidationError::from(format!(
                        "AI-upscale profile {:?} timed out: {}",
                        profile.name,
                        String::from_utf8_lossy(&stderr)
                    )))
                }
                SupervisedOutcome::NotStarted { .. } | SupervisedOutcome::Stopped { .. } => {
                    unreachable!("the AI-upscale smoke test has no stop callback")
                }
            };
            let mut shader = profile.shader_file.as_ref();
            shader.seek(SeekFrom::Start(0)).map_err(|error| {
                ConfigValidationError::from(format!(
                    "cannot rewind AI-upscale shader {:?}: {error}",
                    profile.name
                ))
            })?;
            if !output.status.success() {
                return Err(ConfigValidationError::from(format!(
                    "AI-upscale profile {:?} failed its Vulkan/NVENC smoke test: {}",
                    profile.name,
                    String::from_utf8_lossy(&output.stderr)
                )));
            }
            notes.push(format!(
                "AI upscale {} sha256={} envelope={}x{} @ {} source pixels/s",
                profile.name,
                profile.shader_sha256,
                profile.max_source_width,
                profile.max_source_height,
                profile.max_source_pixels_per_second
            ));
        }
        Ok(notes)
    }

    /// Open runtime storage, perform startup maintenance, and construct the
    /// application from this validated configuration.
    pub fn into_app(self) -> Result<App, AppInitError> {
        let Self {
            cfg,
            mut media_roots,
            configured_uuid,
        ...
        let remaps = cfg.remap.clone();
        let uuid = match configured_uuid {
            Some(uuid) => uuid,
            None => {
                load_or_create_uuid(&cache_dir, None).map_err(|message| AppInitError::Identity {
                    path: cache_dir.clone(),
                    message,
                })?
            }
        ...
        if !derived_startup.quota_satisfied {
            return Err(AppInitError::DerivedImageCache {
                ...
                source: std::io::Error::other(
                    "derived-image cache cannot satisfy its configured quota",
                ),
            });
        }
        ...
        let ai_upscale_max_jobs = cfg.web.ai_upscale_max_jobs as usize;
...
impl App {
    ...
    fn handle_from_inner(&self, req: &HttpRequest, peer: SocketAddr) -> HttpResponse {
        ...
        if req
            .headers
            .iter()
            .filter(|(name, _)| name.eq_ignore_ascii_case("SOAPAction"))
            .nth(1)
            .is_some()
        {
            return HttpResponse::html(400, "Bad Request", "duplicate SOAPAction");
        }
    ...
    fn gena(&self, req: &HttpRequest, persist: bool, peer: SocketAddr) -> HttpResponse {
        ...
        if let Some(raw_cb) = cb_hdr {
            ...
            if let Err(status) = events::subscribe_and_enqueue_initial(
                &self.events,
                ...
                service,
                timeout,
                || events::propertyset(service, self.update_id.load(Ordering::Relaxed)),
            ) {
                let (reason, message) = if status == 503 {
                    ("Service Unavailable", "notification queue full")
                } else {
                    ("Precondition Failed", "subscriber table")
                };
                let mut response = HttpResponse::html(status, reason, message);
                if status == 503 {
    ...
    fn soap(&self, req: &HttpRequest, persist: bool, peer: SocketAddr) -> HttpResponse {
        ...
        if !is_search {
            ...
            let (Some(starting_index), Some(requested_count)) =
                (call.starting_index, call.requested_count)
            else {
                return soap_fault_logged(
                    SoapOutcome::fault402(),
                    persist,
            ...
            if starting_index < 0 || requested_count < 0 {
                return soap_fault_logged(
                    SoapOutcome::fault402(),
                    persist,
                    &call,
            ...
            let (Ok(start), Ok(requested_count)) = (
                usize::try_from(starting_index),
                usize::try_from(requested_count),
            ) else {
                return soap_fault_logged(
                    SoapOutcome::fault402(),
                    persist,
            ...
            let take = if requested_count == 0 {
                MAX_SOAP_PAGE_OBJECTS
            } else {
                requested_count.min(MAX_SOAP_PAGE_OBJECTS)
            };
            ...
            let db_page = direct_sort.as_ref().and_then(|sort| {
                self.cached_catalog_query_page(query_generation, &query_cache_key)
                    ...
                    .or_else(|| {
                        query_stable_catalog_snapshot(self, || {
                            query_db_children(
                ...
                if flag == "BrowseMetadata" {
                    ...
                    if let Some((objects, total)) = db_page.as_ref().and_then(|snapshot| {
                        (snapshot.generation == catalog_generation
                            ...
                            .then(|| {
                                materialize_db_page(&cat, &snapshot.page).map(|items| {
                                    self.cache_catalog_query_page(
                                        catalog_generation,
                                        query_cache_key.clone(),
                                        snapshot.page.clone(),
                                    );
                                    (items, snapshot.page.total)
                                })
                            })
                            .flatten()
                    }) {
        ...
        let (didl, total, response_generation) = {
            ...
            if let Some(snapshot) = db_page
                .as_ref()
                .filter(|snapshot| snapshot.generation == catalog_generation)
            {
                if let Some(objects) = materialize_db_page(&cat, &snapshot.page) {
                    self.cache_catalog_query_page(
                        catalog_generation,
                        query_cache_key,
                        snapshot.page.clone(),
                    ...
                    let (didl, total) = search_memory_page(
                        self,
                        &cat,
                        &scope,
                        &clauses,
        ...
        match child {
            ...
            CatalogChildRef::Item(it) => {
                ...
                DidlObject {
                    ...
                    // outline; the full plot belongs to the web reveal.
                    description: it.about.clone(),
    ...
    fn persist_bookmark(
        ...
        // generation. Do not hold the catalog write lock while waiting for
        // SQLite: Browse remains available during writer contention. Kodi
        // caches Browse results and invalidates them from ContainerUpdateIDs
        // rather than SystemUpdateID alone.
        let _publication = lock_recover(&self.catalog_publication);
        let update_id = rusty_dlna_protocol::soap::next_system_update_id(
            self.update_id.load(Ordering::Acquire),
        );
    ...
    pub(crate) fn media(
        ...
        if transcode {
            ...
            if plan.decision != Decision::Recode {
                ...
                let mut remux_p8 = plan.action == RecodeAction::RemuxP8;
                if remux_p8 && dovi_tool_path().is_none() {
                ...
                let cache_identity = match transcode_cache_identity_file_controlled(
                    &opened.file,
                    ...
                    Err(error) => {
                        tracing::warn!(%error, "transcode tool fingerprint unavailable");
                        let message = match error {
                            ToolQueryError::Busy(_) => "media helper busy",
                            ToolQueryError::Cancelled => "server is stopping",
                            ToolQueryError::Deadline { .. } | ToolQueryError::Query { .. } => {
                                "transcode tool unavailable"
                            }
                        };
                        let mut response = HttpResponse::html(503, "Service Unavailable", message);
                        response.set("Retry-After", "1");
                        return response;
                    }
                };
                ...
                let profile8_toolchain = cache_identity.profile8_toolchain().cloned();
                debug_assert_eq!(remux_p8, profile8_toolchain.is_some());
                let dest =
    ...
    fn resized(&self, req: &HttpRequest) -> HttpResponse {
        ...
        let _active_image = self.derived_images.activate(&key, &self.cache_maintenance);
        if !dest.is_file() {
            let cache_quota = self.cfg.derived_image_cache_mb * 1024 * 1024;
            let minimum_free = self.cfg.cache_min_free_mb * 1024 * 1024;
            let pre_maintenance = self.derived_images.maintain(
            ...
            match pre_maintenance {
                Ok(report) if report.limits_satisfied => {}
                Ok(report) => {
                    tracing::warn!(
                        cache_bytes = report.bytes,
                        "derived-image cache cannot satisfy a configured limit"
                    );
            ...
            let post_maintenance = self.derived_images.maintain(
                &self.cache_maintenance,
                &derived_dir,
                cache_quota,
                self.cfg.derived_image_cache_age_days,
...
pub(crate) fn media_read_error_response(
    ...
    tracing::error!(path = %path.display(), title, %error, "media read failed");
    HttpResponse::html(
        500,
        "Internal Server Error",
