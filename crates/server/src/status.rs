...
use rusty_dlna_protocol::escape_xml;
...
fn catalog_counts(app: &App) -> CatalogCounts {
    ...
    for item in cat.items.values() {
        ...
        counts.captions = counts
            .captions
            .saturating_add(u32::try_from(item.captions.len()).unwrap_or(u32::MAX));
...
fn status_value(app: &App, detailed: bool) -> (Health, Value) {
    ...
    let web_player = json!({
        ...
        "prepared_reuses_total": remux.web_prepared_reuses_total,
        ...
        // initial output bytes rather than playback.
...
pub fn status_html(app: &App) -> String {
    ...
    format!(
        ...
        name = escape_xml(&app.cfg.friendly_name),
        ...
        phase = escape_xml(scanner["phase"].as_str().unwrap_or("idle")),
