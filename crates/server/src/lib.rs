...
use rusty_dlna_helper::{
    duration_millis_saturating, read_to_end_bounded, BoundedReadError, CancellationToken,
    HelperGate, JobGate,
};
use rusty_dlna_http::{
...
use rusty_dlna_scan::{
    build_media_roots, caption_http_mime, load_and_persist_media_root_mappings, load_existing,
    run_inotify_prepared_updates_until, Catalog, CatalogChild, CatalogUpdate,
    Container as CatalogContainer, LibraryDb, MediaItem, MediaRoot, MediaTypes,
    PreparedCatalogChange, ScanConfig, ScanProgress, ScanResult, ScanSession, SourceProbe,
    WatchTelemetry,
};
...
use rusty_dlna_transcode::{
    ...
    ffmpeg_grow_os_args, hdr10_fallback_plan, pick_audio_index_from_streams, probe_to_source,
    source_identity_file, transcode_cache_identity_file_controlled, AudioAction, BrowserAiUpscale,
    Decision, HardwareDecode, HdrKind, RecodeAction, RemapRule, ToolQueryControl, ToolQueryError,
    TranscodePlan,
};
...
pub(crate) use cache_fs::available_filesystem_bytes;
use catalog_query::*;
use config::{command_version, load_or_create_uuid, normalize_uuid, validate_http_config};
pub use config::{
...
#[cfg(test)]
use derived_image_cache::prune_derived_image_cache;
#[cfg(test)]
use http_app::{fetch_renderer_description, renderer_xml_body, MAX_RENDERER_DESCRIPTION_BYTES};
use http_app::{
...
pub struct App {
    ...
    /// Serializes persisted and in-memory catalog generations without making
    /// read-only catalog users wait for SQLite writer admission.
    ...
    derived_images: derived_image_cache::DerivedImageCache,
...
/// A fully validated application configuration that has not opened or changed
/// the catalog, identity, root mappings, or caches.
///
/// Diagnostic callers can inspect this value without triggering runtime
/// startup. Call [`AppPreflight::into_app`] to construct the mutable runtime
/// state after diagnostics have completed.
pub struct AppPreflight {
    cfg: Config,
    media_roots: Vec<MediaRoot>,
    configured_uuid: Option<String>,
    http_port: u16,
    ssdp_port: u16,
    listen_ip: Ipv4Addr,
    advertise_ip: String,
    ssdp_interfaces: Vec<(Ipv4Addr, Ipv4Addr)>,
    cache_dir: PathBuf,
    db_path: PathBuf,
    ai_upscale_profiles: Vec<BrowserAiUpscaleProfile>,
}

#[derive(Debug)]
pub(crate) struct BrowserAiUpscaleProfile {
    name: String,
    shader_file: Arc<std::fs::File>,
...
impl AppPreflight {
    /// Return the validated user configuration.
    pub fn config(&self) -> &Config {
        &self.cfg
    }

    /// Return the resolved HTTP listener port.
    pub fn http_port(&self) -> u16 {
        self.http_port
    }

    /// Return the resolved SSDP listener port.
    pub fn ssdp_port(&self) -> u16 {
        self.ssdp_port
    }

    /// Return the validated HTTP listener address.
    pub fn listen_ip(&self) -> Ipv4Addr {
        self.listen_ip
    }

    /// Return the selected advertisement address.
    pub fn advertise_ip(&self) -> &str {
        &self.advertise_ip
    }

    /// Return the configured UUID, normalized for the wire, when present.
    pub fn configured_uuid(&self) -> Option<&str> {
        self.configured_uuid.as_deref()
    }

    /// Resolve the identity for a read-only diagnostic. A configured UUID is
    /// preferred; otherwise an existing persisted UUID is read and validated.
    /// A fixed valid UUID is used only when no persisted identity exists.
    pub fn diagnostic_uuid(&self) -> Result<String, AppInitError> {
        const CHECK_UUID: &str = "uuid:00000000-0000-4000-8000-000000000001";
        if let Some(uuid) = &self.configured_uuid {
            return Ok(uuid.clone());
        }
        let path = self.cache_dir.join("uuid");
        match std::fs::read_to_string(&path) {
            Ok(stored) => normalize_uuid(&stored).map_err(|message| AppInitError::Identity {
                path: self.cache_dir.clone(),
                message: format!("invalid persisted UUID: {message}"),
            }),
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => Ok(CHECK_UUID.to_owned()),
            Err(error) => Err(AppInitError::Identity {
                path: self.cache_dir.clone(),
                message: format!("cannot read persisted UUID {}: {error}", path.display()),
            }),
        }
    }
    ...
    /// Return the resolved catalog database path.
    pub fn database_path(&self) -> &Path {
        &self.db_path
    }

    /// Return the resolved SSDP notification interval.
    pub fn notify_interval(&self) -> u32 {
        self.cfg.notify_interval.unwrap_or(895)
    }
}

impl App {
    fn notify_catalog_publication(
        &self,
        _published: &std::sync::RwLockWriteGuard<'_, Catalog>,
        update_id: u32,
        ...
        if let Some(parent_ids) = parent_ids {
            ...
            events::notify_content_dir(&self.events, &self.notify_dispatcher, update_id);
        }
    }
}
...
/// Serialize an application response through the fallible wire API so an
/// invalid public field is visible in logs. The boolean is false when the
/// returned bytes are the fixed fallback and no external body may follow.
pub(crate) fn response_wire(app: &App, response: &HttpResponse) -> (Vec<u8>, bool) {
    let date = now_imf_date();
    match response.try_bytes_wire(&app.server, &date) {
        Ok(wire) => (wire, true),
        Err(error) => {
            tracing::error!(%error, status = response.status, "refused invalid HTTP response");
            (response.bytes_wire(&app.server, &date), false)
        }
    }
}

/// Write one complete response header/in-memory body and report whether
/// descriptor-backed or growing bytes may safely follow it.
pub(crate) async fn socket_write_http_response(
    app: &App,
...
pub enum AppInitError {
    ...
    #[error("cannot maintain derived-image cache {path}: {source}")]
    DerivedImageCache {
        path: PathBuf,
        #[source]
        source: std::io::Error,
    },
    #[error("cannot load web AI-upscale shader {path}: {message}")]
    AiUpscaleShader { path: PathBuf, message: String },
...
struct DbReaderLease<'a> {
    pool: &'a DbPool,
    db: Option<LibraryDb>,
    failed: bool,
}

impl DbReaderLease<'_> {
    fn db(&self) -> &LibraryDb {
        self.db.as_ref().expect("reader lease owns a connection")
    }
}
...
impl Drop for DbReaderLease<'_> {
    fn drop(&mut self) {
        ...
        if self.failed {
            self.pool.errors_total.fetch_add(1, Ordering::Relaxed);
        }
...
struct DbWriterActivity<'a> {
    pool: &'a DbPool,
    failed: bool,
}

impl DbWriterActivity<'_> {
    fn new(pool: &DbPool) -> DbWriterActivity<'_> {
        pool.writer_active.store(true, Ordering::Release);
        DbWriterActivity { pool, failed: true }
    }
}

impl Drop for DbWriterActivity<'_> {
    fn drop(&mut self) {
        self.pool.writer_active.store(false, Ordering::Release);
        self.pool.writes_total.fetch_add(1, Ordering::Relaxed);
        if self.failed {
            self.pool.errors_total.fetch_add(1, Ordering::Relaxed);
        }
    }
...
impl DbPool {
    ...
    fn read<T>(
        ...
        if waited {
            ...
            let wait_ms = duration_millis_saturating(wait_started.elapsed());
    ...
    fn write<T>(
        ...
        let db = self
            ...
            .lock()
            .unwrap_or_else(|error| error.into_inner());
        let mut activity = DbWriterActivity::new(self);
        let result = query(&db);
        activity.failed = result.is_err();
        result
    }
    ...
    fn write_scan<T, E>(
        &self,
        cancellation: &CancellationToken,
        query: impl FnOnce(&LibraryDb) -> Result<T, E>,
    ) -> ScanResult<T>
    where
        E: Into<rusty_dlna_scan::ScanError>,
    {
        let db = loop {
    ...
    fn reopen_writer(&self, cancellation: &CancellationToken) -> rusqlite::Result<()> {
        let mut writer = self
            .writer
            .lock()
            .unwrap_or_else(|error| error.into_inner());
        let path = writer.path.clone();
        *writer = LibraryDb::open_with_cancellation(&path, cancellation.clone())?;
        Ok(())
    }
    ...
    fn get(self: &Arc<Self>, pool: Option<Arc<DbPool>>) -> DbIntegritySnapshot {
            ...
            && !self.refresh_in_flight.swap(true, Ordering::AcqRel)
        {
            ...
            let spawn = std::thread::Builder::new()
                ...
                .spawn(move || {
                    let _refresh = RefreshInFlightGuard::new(refreshing);
                    let started = Instant::now();
                    let result = check_database_integrity(&pool);
                    let duration_ms = duration_millis_saturating(started.elapsed());
