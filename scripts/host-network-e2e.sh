...
if [[ ${RUSTY_DLNA_REQUIRE_E2E:-} != 1 ]]; then
  echo "host-network-e2e: set RUSTY_DLNA_REQUIRE_E2E=1 to use the live SSDP port" >&2
  exit 77
fi
...
expected="LOCATION: http://%s:%d/rootDesc.xml"%(local,http_port)
...
while time.monotonic()<deadline:
    ...
    try:
        ...
        continue
    if peer[0]==local and expected in text:
        break
else:
    raise TimeoutError("no matching SSDP reply after repeated M-SEARCH probes")
print("peer=%s:%s"%peer)
print(text)'
