...
This matrix is the supported contract for rustyDLNA. “Compatible” means the
corresponding MiniDLNA object IDs, UPnP actions, resource MIME types, and client
quirks are regression-tested; it does not mean every MiniDLNA daemon option or
optional discovery protocol is implemented.

| Area | Status | Contract / intentional difference |
|---|---|---|
| Video library | Supported | First-class scanning, NFO, subtitles, artwork, Browse/Search, original GET, and opt-in remap/transcode. |
...
| Picture library | Supported for JPEG | EXIF date/camera/orientation/rating/album, oriented derivatives, Date/Camera/Album/Rating/Recent/folder/playlist views. JPEG/JPEG_TN is the MiniDLNA parity target. |
...
| Filesystem upload/import API | Out of scope | Neither rustyDLNA nor the MiniDLNA reference exposes a supported upload/import endpoint. Media enters through configured read-only roots. |
...
| MiniDLNA-style option | rustyDLNA decision |
|---|---|
| `friendly_name`, `port`, `network_interface`, `media_dir`, `exclude_dir`, `root_container`, `max_connections` | Supported with strict startup validation. `media_dir` type prefixes are per root. |
...
| `strict_dlna` | Not a global switch. The inherited strict checks and client exceptions are applied by identified renderer profile. |
...
file, and `--print-effective-config` provides a secret-free resolved view.
...
assumption of feature-for-feature daemon parity.
