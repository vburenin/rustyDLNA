...
| Original-file socket/CPU experiment | `python3 scripts/direct-delivery-benchmark.py --help` | `docs/OPERATIONS.md`; saved release test executables, generated bytes, isolated loopback ports |
| Native EVENT history/decoder measurement (additional native-device validation required) | `RUSTY_DLNA_NATIVE_EVIDENCE=/tmp/native-history-evidence cargo test --release --locked -p rusty-dlna --lib measure_native_variable_gop_history -- --ignored --nocapture` | `docs/WEB_PLAYER.md`; generated long variable-GOP media |
